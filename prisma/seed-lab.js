...
const PROJECTS = [
  {
    ...
    dataSources: [
      { name: 'Line 3 Vibration Sensors (IIoT Gateway)', type: 'IIOT_SENSOR', status: 'CONNECTED', recordsIngested: 4820000 },
      { name: 'Line 3 Temperature & Acoustic Array', type: 'IIOT_SENSOR', status: 'CONNECTED', recordsIngested: 3140000 },
      { name: 'Plex MES — Work Order History', type: 'MES', status: 'CONNECTED', recordsIngested: 218400 },
      { name: 'SAP ERP — Asset Master Data', type: 'ERP', status: 'CONNECTING', recordsIngested: 0 },
    ],
    pipelines: [
      { name: 'Raw Sensor Ingestion', stage: 'INGESTION', status: 'ACTIVE', recordsProcessed: 4820000 },
      { name: 'Sensor Noise Cleaning', stage: 'CLEANING', status: 'ACTIVE', recordsProcessed: 4610000 },
      { name: 'RUL Feature Engineering', stage: 'FEATURE_ENGINEERING', status: 'ACTIVE', recordsProcessed: 890000 },
      { name: 'Failure Event Labeling', stage: 'LABELING', status: 'DRAFT', recordsProcessed: 0 },
    ],
    datasets: [
  ...
  },
  {
    ...
    dataSources: [
      { name: 'Inspection Station Camera Feed', type: 'VISION_CAMERA', status: 'CONNECTED', recordsIngested: 182400 },
      { name: 'Manual Inspection Log (parallel run)', type: 'FILE_UPLOAD', status: 'CONNECTED', recordsIngested: 41200 },
    ],
    pipelines: [
      { name: 'Image Ingestion & Calibration Check', stage: 'INGESTION', status: 'ACTIVE', recordsProcessed: 182400 },
      { name: 'Defect Labeling Queue', stage: 'LABELING', status: 'ACTIVE', recordsProcessed: 38600 },
      { name: 'Augmentation Pipeline', stage: 'TRANSFORMATION', status: 'DRAFT', recordsProcessed: 0 },
    ],
    datasets: [
  ...
  },
  {
    ...
    dataSources: [
      { name: 'SAP ERP — Purchase Order History', type: 'ERP', status: 'CONNECTED', recordsIngested: 96400 },
      { name: 'Supplier Portal — Lead Time Feed', type: 'API_FEED', status: 'ERROR', recordsIngested: 0 },
    ],
    pipelines: [
      { name: 'Consumption History Ingestion', stage: 'INGESTION', status: 'ACTIVE', recordsProcessed: 96400 },
      { name: 'Supplier Reliability Cleaning', stage: 'CLEANING', status: 'DRAFT', recordsProcessed: 0 },
    ],
    datasets: [],
    trainingJobs: [],
    models: [],
  },
  {
    ...
    dataSources: [
      { name: 'MES Work Order Parameter Log', type: 'MES', status: 'CONNECTING', recordsIngested: 0 },
    ],
    pipelines: [],
    datasets: [],
    trainingJobs: [],
